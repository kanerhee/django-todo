    ...
    if request.method == 'GET':
        ...
        if request.POST['password1'] == request.POST['password2']:
            try:
                ...
                # tell the user that that username has already been taken, and send them back to the same page
                return render(request, 'todo/signupuser.html', {'form':UserCreationForm(), 'error':'That username has already been taken '})
        else:
            # tell the user the passwords didn't match, and send them back to the same page
...
def logoutuser(request):
    # only want to log someone out if request is a POST
...
def createtodo(request):
    if request.method == 'GET':
        ...
        try:
            ...
            # in case they fill out too large of field or etc.
